...
The IndexedDB API supports transactions that rely on strict two-phase locking, so that read-only transactions can run concurrently but read-write transactions lock the specified object stores (similar to tables in a relational data store) for the entire duration of the transaction.
